...
class Cfg(dict):
    ...
    @staticmethod
    def load_config():
        with open('./vietocr/cfg/base.yaml', encoding='utf-8') as f:
            base_config = yaml.safe_load(f)
        
        with open('./vietocr/cfg/vgg_transformer.yaml') as f1:
            config = yaml.load(f1)
        # config = yaml.safe_load('/home/hung-vt/Documents/CMND/vietocr/cfg/vgg_transformer.yaml')
        config.update(base_config)
        return Cfg(config)
